...
多Agent，不是一个模型把所有事情都做完，而是多个模型分工协作。多个 Agent 一起合作解决问题，每个智能体负责不同的任务和能力。Multi-Agent 就是把这个思想搬到 AI 里。



使用子图构建多Agent系统，整体架构如下：
...
flowchart TD
    ...
    subgraph TravelGraph[主 Agent：travelGraph]
        START --> IntentNode[意图识别 intent_node]

        IntentNode -->|weather| WeatherNode[weather 子图调用]
        IntentNode -->|ticket| TicketNode[ticket 子图调用]
        IntentNode -->|chat| ChatNode[闲聊 chat]

        WeatherNode --> ResponseNode[response]
        TicketNode --> ResponseNode
        ChatNode --> END

        ResponseNode --> END
    end

    subgraph WeatherGraph[子 Agent：weatherGraph]
        WQ[weather_query]
        WQ --> WT[weather_tool]
        WT --> WR[(weather DB)]
    end

    subgraph TicketGraph[子 Agent：ticketGraph]
        TQ[ticket_query]
        TQ --> TT[ticket_tool]
        TT --> TR[(ticket DB)]
    end

    WeatherNode --> WeatherGraph
    TicketNode --> TicketGraph
...
stateDiagram-v2
    ...
    intent_node --> weather : intent = weather
    intent_node --> ticket : intent = ticket
    intent_node --> chat : intent = chat
...
flowchart LR
    StartW[Start]
    StartW --> weather_query
    weather_query --> weather_tool
    weather_tool --> EndW[End]
